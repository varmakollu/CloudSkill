...
gcloud compute instances create web1 --project=$DEVSHELL_PROJECT_ID --zone=$ZONE --machine-type=e2-small --tags=network-lb-tag --image-family=debian-11 --image-project=debian-cloud \
...
gcloud compute instances create web2 --project=$DEVSHELL_PROJECT_ID --zone=$ZONE --machine-type=e2-small --tags=network-lb-tag --image-family=debian-11 --image-project=debian-cloud \
--metadata=startup-script='#!/bin/bash
apt-get update
apt-get install apache2 -y
service apache2 restart
...
gcloud compute instances create web3 --project=$DEVSHELL_PROJECT_ID --zone=$ZONE --machine-type=e2-small --tags=network-lb-tag --image-family=debian-11 --image-project=debian-cloud \
--metadata=startup-script='#!/bin/bash
apt-get update
apt-get install apache2 -y
service apache2 restart
...
gcloud compute instance-templates create lb-backend-template --project=$DEVSHELL_PROJECT_ID --region=$REGION --network=default --subnet=default --tags=allow-health-check --machine-type=e2-medium --image-family=debian-11 --image-project=debian-cloud --metadata=startup-script='#!/bin/bash
     apt-get update
     apt-get install apache2 -y
