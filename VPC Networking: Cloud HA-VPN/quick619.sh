...
--network vpc-demo --range 10.2.1.0/24 --region=$REGION
...
gcloud compute instances create vpc-demo-instance1 --zone=$ZONE2 --subnet vpc-demo-subnet1


gcloud compute instances create vpc-demo-instance2 --zone=$ZONE --subnet vpc-demo-subnet2
...
gcloud beta compute vpn-gateways describe vpc-demo-vpn-gw1 --region $REGION1 

gcloud beta compute vpn-gateways describe on-prem-vpn-gw1 --region $REGION1
...
timeout 45 gcloud compute ssh on-prem-instance1 --zone $ZONE1 --quiet --command="ping 10.1.1.2"
...
echo "Please like share and subscribe to tutorialboy, https://www.youtube.com/@tutorialboy24!"
